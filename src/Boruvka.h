...
namespace joanapl {
    ...
    struct NaiveBoruvka {
        ...
        algen::WEdgeList calculateMST(const algen::WEdgeList &edge_list, const unsigned long num_vertices) {
            ...
            components = *(new UnionFind<uint64_t> (num_vertices));
            ...
            while(not completed) {
                ...
                if(!completed){
                    for (int i = 0; i < vertices.size(); ++i) {
                        ...
                        if(cheapest_edges.at(i).weight != 0 && !edge_is_in_vector(cheapest_edges.at(i), forest_edges)){
        ...
        void add_inverted_edges() {
            ...
            for (algen::VertexId i = 0; i < mst_one_way_size; ++i) {
                (forest_edges).emplace_back((forest_edges)[i].head, (forest_edges)[i].tail, (forest_edges)[i].weight);
            }
