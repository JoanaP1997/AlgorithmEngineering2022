...
namespace joanaplewnia {

    algen::WEdgeList forest_edges;
    bool completed;
    std::vector<uint64_t> vertices;
    joanaplewni::FastUnionFind<uint64_t> components(0);
    ...
    struct FasterBoruvkaOne {
        ...
        algen::WEdgeList calculateMST(const algen::WEdgeList &edge_list, const unsigned long num_vertices, algen::WEdgeList& forest_edges) {
            ...
            if(forest_edges.empty()) {
                ...
                components = *(new joanaplewni::FastUnionFind<uint64_t> (num_vertices)); //only new components if we are not in one filter run
            }
            completed = false;
        ...
        void delete_edges_in_one_component(joanaplewni::FastUnionFind<uint64_t>& components, algen::WEdgeList& edges){
            int max = edges.size();
            int k = 0;
            int j = 0;
                for (int i = 0; i < max; ++i) {
